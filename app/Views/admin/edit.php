...
<section class="mx-8 mt-16 md:mx-16 lg:mt-8 lg:ml-16 lg:mr-24">
    <h1 class="font-bold text-xl mb-2">Edit Post</h1>
    <hr class="border-slate-400">
    <div class="mt-2 grid md:grid-cols-12 gap-3 ">
        <form action="update" method="GET" class="grid gap-3 col-span-10">
            <?= csrf_field(); ?>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:colspan10">
                <label for="" class="mb-1 font-semibold">Id Post</label>
                <input type="text" name="kode_posting" value="<?= $kode_posting; ?>" placeholder="Masukan Kode Postingan" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 md:p-[10px] md:rounded-md" autofocus>
            </div>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
                ...
                <select name="id_admin" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 focus:ring-2 focus:ring-purple-500 md:p-[10px] md:rounded-md">
                    ...
                    <?php foreach ($admin as $index => $row) : ?>
                        <option name="id_admin" value="<?= $index + 1; ?>"><?= $row['kode_admin']; ?> - <?= $row['nama_admin']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
                ...
                <select name="id_produk" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 focus:ring-2 focus:ring-purple-500 md:p-[10px] md:rounded-md">
                    ...
                    <?php foreach ($produk as $index => $row) : ?>
                        <option name="id_produk" value="<?= $index + 1; ?>"><?= $row['kode_produk']; ?> - <?= $row['nama_produk']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
                <label for="" class="mb-1 font-semibold">Cover Latter</label>
                <input type="text" name="cover_latter" value="<?= $cover_latter; ?>" placeholder="Masukan Cover Latter" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 focus:ring-2 focus:ring-purple-500 md:p-[10px] md:rounded-md">
            </div>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
                <label for="" class="mb-1 font-semibold">Deskripsi</label>
                <input type="text" name="desk_produk" value="<?= $desk_produk; ?>" placeholder="Masukan Deskripsi" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 focus:ring-2 focus:ring-purple-500 md:p-[10px] md:rounded-md">
            </div>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
                <label for="" class="mb-1 font-semibold">Gambar</label>
                <input type="file" name="visual_produk" value="<?= $visual_produk; ?>" placeholder="Masukan Gambar" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 focus:ring-2 focus:ring-purple-500 md:p-[10px] md:rounded-md">
            </div>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
                <label for="" class="mb-1 font-semibold">Tanggal</label>
                <input type="date" name="tanggal" value="<?= $tanggal; ?>" placeholder="Masukan Tanggal" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 focus:ring-2 focus:ring-purple-500 md:p-[10px] md:rounded-md">
            </div>
            <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
                <label for="" class="mb-1 font-semibold">Jam</label>
                <input type="time" name="jam" value="<?= $jam; ?>" placeholder="Masukan Jam" class="outline-none border border-slate-900 rounded p-[6px] pl-3 text-sm col-span-2 focus:ring-2 focus:ring-purple-500 md:p-[10px] md:rounded-md">
            </div>
            <!-- <div class="grid grid-cols-3 gap-6 gap-y-1 items-center lg:grid-cols-">
            ...
            <div class="grid gap-6 gap-y-1 items-center lg:grid-cols- ">
                <button type="submit" name="submit" class="w-full my-6 mb-8 py-2 bg-purple-700 rounded-lg text-white font-semibold lg:col-span-3">Edit Post</button>
            </div>
        </form>
